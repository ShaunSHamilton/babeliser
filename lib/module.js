import {parse as $iPNDK$parse} from "@babel/parser";
import $iPNDK$babelgenerator from "@babel/generator";
import {is as $iPNDK$is} from "@babel/types";




class $149c1bd638913645$export$ec4e4cf3d90ab517 {
    maxScopeDepth = 4;
    constructor(codeString, options){
        this.parsedCode = (0, $iPNDK$parse)(codeString, {
            sourceType: "module",
            ...options
        });
    ...
    getExpressionStatement(name, scope = [
        ...
        const expressionStatement = expressionStatements.find((e)=>{
            ...
            if ((0, $iPNDK$is)("CallExpression", expression)) {
                if (name.includes(".")) {
                    const [objectName, methodName] = name.split(".");
                    const memberExpression = expression.callee;
                    if ((0, $iPNDK$is)("MemberExpression", memberExpression)) {
                        const object = memberExpression.object;
                        const property = memberExpression.property;
                        if ((0, $iPNDK$is)("Identifier", object) && (0, $iPNDK$is)("Identifier", property)) return object.name === objectName && property.name === methodName;
                    }
                }
                const identifier = expression.callee;
                if ((0, $iPNDK$is)("Identifier", identifier) && identifier.name === name) return true;
            }
            if ((0, $iPNDK$is)("AwaitExpression", expression)) {
                const callExpression = expression.argument;
                if ((0, $iPNDK$is)("CallExpression", callExpression)) {
                    const identifier = callExpression.callee;
                    if ((0, $iPNDK$is)("Identifier", identifier)) return identifier.name === name;
                }
            }
            return false;
        });
    ...
    generateCode(ast, options) {
        return (0, $iPNDK$babelgenerator).default(ast, options).code;
    }
...
export {$149c1bd638913645$export$ec4e4cf3d90ab517 as Babeliser};
